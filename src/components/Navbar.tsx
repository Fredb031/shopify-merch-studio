...
export function Navbar({ onOpenCart, onOpenLogin }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[400] h-[58px] flex items-center justify-between px-6 md:px-10 bg-background/[0.93] backdrop-blur-xl border-b border-border" role="navigation" aria-label="Main navigation">
      <Link to="/" aria-label="Vision Affichage — Home">
        <img src="https://cdn.shopify.com/s/files/1/0578/1038/7059/files/Asset_1_d5d82510-0b83-4657-91b7-3ac1992ee697.svg?height=90&v=1769614651" alt="Vision Affichage" className="h-6" />
      </Link>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={onOpenCart} aria-label={`${t('panier')}${itemCount > 0 ? ` (${itemCount})` : ''}`} className="flex items-center gap-[7px] text-[13px] text-muted-foreground border border-border px-4 py-[7px] rounded-full transition-all hover:border-muted-foreground hover:text-foreground relative">
