/**
 * ProductViewer3D — Clean static 3D product viewer
 *
 * - NO rotation / drag / OrbitControls
 * - NO background texture image
 * - Devant / Dos toggle only
 * - Solid fabric material with selected colour
 * - Logo placed directly on 3D surface
 * - Centred, fixed, beautiful
 */
...
// ── Fabric material — solid colour, no texture ─────────────────────────────
...
function HoodieModel({ hex, logoUrl, logoPlacement }: { hex: string; logoUrl?: string; logoPlacement?: LogoPlacement }) {
  ...
  const darkC     = new THREE.Color(hex).multiplyScalar(0.72);
  const midC      = new THREE.Color(hex).multiplyScalar(0.80);
  const darkMat   = useMemo(() => new THREE.MeshStandardMaterial({ color: darkC, roughness: 0.92 }), [hex]);
  const midMat    = useMemo(() => new THREE.MeshStandardMaterial({ color: midC, roughness: 0.88 }), [hex]);
  const cuffMat   = useMemo(() => new THREE.MeshStandardMaterial({ color: new THREE.Color(hex).multiplyScalar(0.76), roughness: 0.94 }), [hex]);
  ...
  return (
    <group position={[0, -0.06, 0]}>
      ...
      <mesh geometry={hoodGeo} material={midMat} />
      <mesh geometry={pocketGeo} material={darkMat} />
...
// ── Generic T-Shirt ─────────────────────────────────────────────────────────
function ShirtModel({ hex, logoUrl, logoPlacement }: { hex: string; logoUrl?: string; logoPlacement?: LogoPlacement }) {
...
function CapModel({ hex, logoUrl }: { hex: string; logoUrl?: string }) {
  ...
  const trim  = useMemo(() => new THREE.MeshStandardMaterial({
    color: new THREE.Color(hex).lerp(new THREE.Color('#888'), 0.28),
    roughness: 0.82,
  }), [hex]);
  return (
...
function BeanieModel({ hex, logoUrl }: { hex: string; logoUrl?: string }) {
  ...
  const cuffMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: new THREE.Color(hex).multiplyScalar(0.80),
    roughness: 0.95,
  }), [hex]);
  return (
    <group>
...
function Skeleton() {
  return (
    <mesh>
      ...
      <meshBasicMaterial color="#E8E6E1" transparent opacity={0.5} />
    </mesh>
  );
}
...
// ── Scene ───────────────────────────────────────────────────────────────────
function Scene({
  product, hex, logoUrl, logoPlacement,
}: {
  product: Product; hex: string; logoUrl?: string; logoPlacement?: LogoPlacement;
}) {
  const cat = product.category;
  const props = { hex, logoUrl, logoPlacement };

  return (
    <>
      {/* 3-point studio lighting — no environment map needed */}
      ...
      <Suspense fallback={<Skeleton />}>
        {cat === 'hoodie'  && <HoodieModel {...props} />}
        {cat === 'tshirt'  && <ShirtModel  {...props} />}
        {cat === 'polo'    && <ShirtModel  {...props} />}
        {cat === 'manteau' && <ShirtModel  {...props} />}
        {cat === 'cap'     && <CapModel    hex={hex} logoUrl={logoUrl} />}
        {cat === 'toque'   && <BeanieModel hex={hex} logoUrl={logoUrl} />}
      </Suspense>
    </>
  );
}
  ...
  return (
    <div className="relative flex flex-col rounded-2xl overflow-hidden bg-[#F4F3EF] border border-border">
      ...
      <div style={{ height: H }} className="relative pointer-events-none">
        ...
        {/* Colour chip */}
        {selectedColor && (
          <div className="absolute bottom-3 left-3 pointer-events-auto flex items-center gap-2 bg-white/85 backdrop-blur-sm rounded-full px-2.5 py-1.5 border border-border shadow-sm">
            <div className="w-3 h-3 rounded-full ring-1 ring-black/10" style={{ background: selectedColor.hex }} />
        ...
        <AnimatePresence>
          {logoPlacement?.previewUrl && (
            <motion.div
              ...
              className="absolute bottom-3 right-3 pointer-events-none bg-green-700/90 text-white text-[10px] font-bold px-2.5 py-1 rounded-full"
            >
              {t('logoPlace')}
            </motion.div>
          )}
